...
---@field source number Players source id set by the server to target that player
...
function Player.new(playerId)
    ...
    local identifiers = {}
    local identifierCount = GetNumPlayerIdentifiers(tostring(playerId))
    local foundPrimaryIdentifier = false

    for i = 1, identifierCount - 1 do
        local identifier = GetPlayerIdentifier(tostring(playerId), i)
    ...
    if not foundPrimaryIdentifier then
        DropPlayer(tostring(playerId), "Couldn't find the primary identifier required for this server! | " .. PrimaryIdentifier .. " |")
        return nil
    end
